...
int
getChallenge(uint8_t **salt,uint32_t *salt_len, uint8_t **key, uint32_t *key_len)
{
    Srp cli;
...
int
verifySession(uint8_t **serverKeyProof, uint16_t *proof_len, uint8_t *clientPublicKey, uint16_t client_key_len, uint8_t *serverPublicKey, uint16_t server_key_len)
{
    ...
    byte clientPubKey[384]; /* A */
//    byte serverPubKey[80]; /* B */
    word32 clientPubKeySz = 384;
//    word32 serverPubKeySz = 80;
    ...
    if (!r) r = wc_SrpComputeKey(&cli, clientPubKey, clientPubKeySz,
                                 serverPublicKey, server_key_len);
    if (!r) r = wc_SrpGetProof(&cli, clientProof, &clientProofSz);

    *serverKeyProof = clientProof;
    *proof_len = clientProofSz;
